...
3. Select or capture photos. Each photo is scanned at its original resolution; the natural orientation is used first, with rotation attempted only if no serial label is found. Only codes immediately below a `シリアルナンバー` label are read, then receive extra confirmation passes.
...
A full-resolution sparse-text pass finds possible code lines, and a local Japanese OCR check verifies the `シリアルナンバー` label directly above each line. The app tries rotations only as a fallback, then rescans each confirmed serial-line crop with grayscale and locally thresholded variants. It also independently rechecks `2` and `Z` glyphs; a disagreement leaves the result in review instead of silently changing it.
